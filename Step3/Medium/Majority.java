...
/* Moores Voting Algorithm:
 * The idea is to maintain a count of the current candidate for majority element.
 * If the count becomes zero, we set the current element as the new candidate and reset the count to 1.
 * If the current element is equal to the candidate, we increment the count.
 * If it is not equal, we decrement the count.
 * At the end, we check if the candidate is indeed the majority element by counting its occurrences in the array.
*/ 
